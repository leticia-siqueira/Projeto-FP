...
def adicionar_animal(animais, id_atual):
    ...
    print(f"\n===---=== Animal cadastrado com o ID {id_atual}")
    return id_atual + 1
...
def editar_animal(animais):
    ...
    try:
        ...
        print("\nEntrada inválida. Digite um número de ID válido.")
...
def excluir_animal(animais):
    ...
    try:
        ...
        confirmar = input(f"\nTem certeza que deseja excluir '{animais[id_excluir]['nome']}'? (s/n): ").lower()
        if confirmar == "s":
...
def registrar_adocao(animais, lista_adocoes):
    print("\n====== Registrar Adoção ======")
    
    if not animais:
        print("=== Nenhum animal disponível para adoção ===")
        return

    visualizar_animais(animais)
    
    try:
        id_animal = int(input("\nDigite o ID do animal que será adotado: "))
        
        if id_animal not in animais:
            print("=== ID não encontrado ===")
            return
        
        animal = animais[id_animal]
        ...
        lista_adocoes.append(nova_adocao)
        salvar_adocoes(lista_adocoes) 

        del animais[id_animal]
        salvar_animais(animais) 

        print(f"\n=== Sucesso! {animal['nome']} foi adotado por {nome_adotante}! ===")
        print("=== O animal foi movido para o histórico de adoções ===")

    except ValueError:
        print("\nEntrada inválida. Digite um número.")


def sugestao_adocao(animais):

    if not animais:
        print("\nEita, parece que ainda não temos nenhum animal disponível para adoção;-;")
        return
    
    animais_encontrados = []
    
    print("\nOlá Cliente, parece que você deseja participar do nosso quiz para saber qual animal combina mais com você")
    while True:
        opcao = input("Você deseja começar?(s/n): ").lower()

        if opcao != "s" and opcao != "n":
        
            print("Você digitou uma opção inválida\n" \
            "Digite 's' caso queira fazer o quiz, e 'n' caso não queira" )
    ...
    for id_animal,dados in animais.items():
        encontrar_especie = (especie in dados['espécie']) or (especie == "")
        encontrar_comportamento = (comportamento in dados['comportamento']) or (comportamento == "")
        encontrar_idade = (str(idade) in dados['idade']) or (str(idade) == "")

        if encontrar_especie and encontrar_comportamento and encontrar_idade:
            animais_encontrados.append(dados)
        
    if len(animais_encontrados)>0:
        print(f"Encontramos {len(animais_encontrados)} animais/animal compatívies com você!")

        for animal in animais_encontrados:
            print(f"\nNome: {animal['nome']} | Raça: {animal['raça']} | Idade: {animal['idade']} | Comportamento: {animal['comportamento']}")
    else:
...
while True:
    ...
    if opcao == 1:
        ...
        registrar_adocao(animais,lista_adocoes)
    elif opcao == 6:    
        sugestao_adocao(animais)
    elif opcao == 7:
        break
    else:
        print("\n===---=== Opção inválida ===---===")
...
# ----------------------- Cadastro de atividades e cuidados -----------------------
...
def cadastro_cuidados_atividades(lista_servicos):
    ...
    try:
        ...
        salvar_servicos(lista_servicos)
        
        print("\nTarefa cadastrada com sucesso!")
    
    except Exception as e:
        print(f"\nErro no cadastro, tente novamente! {e}")
...
while True:
    try:
        print("\n--------- Outras opções ------------")
        print("\n[1] - Cadastrar outro serviço\n[2] - Encerrar cadastro de atividade")
        outro_cadastro = int(input("Digite a opção que você deseja: "))
        if outro_cadastro == 1:
            cadastro_cuidados_atividades(lista_servicos)
        elif outro_cadastro == 2:
            print("\nObrigada pela preferência! Sua atividade foi marcada.")
            break
        else:
            print("\nResposta inválida!")
            continue
...
contagem_regressiva(animais, lista_servicos)
